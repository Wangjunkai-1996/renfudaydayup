...
from app.domain.legacy_facade import can_use_legacy_bridge, explain_legacy_signal
...
def explain_signal(signal_id: str, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    if _use_legacy(current_user):
        legacy_explain = explain_legacy_signal(signal.id)
        if legacy_explain is not None:
            return {'success': True, 'source': 'legacy', 'signal_id': signal.id, 'explain': legacy_explain}
    ...
    return {'success': True, 'source': 'next', 'signal_id': signal.id, 'explain': payload.model_dump()}
